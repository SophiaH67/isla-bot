...
async fn login(data_dir: &Path, session_file: &Path) -> anyhow::Result<Client> {
    ...
    match client
        ...
        .initial_device_display_name("Isla's Matrix Frontend")
        .send()
        .await
    {
        Ok(_) => {
...
async fn build_client(data_dir: &Path) -> anyhow::Result<(Client, ClientSession)> {
    ...
    let homeserver = env::var("MATRIX_HOMESERVER").expect("MATRIX_HOMESERVER not set");
    println!("\nChecking homeserver…");
...
async fn sync(
    ...
    // Handlers
    client.add_event_handler(events::original_sync_room_message::on_room_message);
    client.add_event_handler(events::stripped_room_member::on_stripped_room_member);
